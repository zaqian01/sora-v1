-- ================= SORA RECORDER V5 (ANTI-DETECTION) =================
local Players = game:GetService("Players")
local HttpService = game:GetService("HttpService")
local TweenService = game:GetService("TweenService")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")

local player = Players.LocalPlayer
local placeId = game.PlaceId
local fileName = "SORA_MAP_" .. placeId .. ".json"

local recordedPoints = {}
local flying = false
local rushing = false
local speeds = {50, 100, 200, 500}
local currentSpeedIdx = 1
local flySpeed = speeds[currentSpeedIdx]

local delays = {1, 3, 5, 10, 30, 60, 100, 160, 320, 500} -- Menambahkan delay pendek untuk testing
local currentDelayIdx = 4
local rushDelay = delays[currentDelayIdx]

-- FUNGSI SAVE/LOAD
local function saveToFile()
    local data = {}
    for _, cf in ipairs(recordedPoints) do table.insert(data, {cf:GetComponents()}) end
    writefile(fileName, HttpService:JSONEncode(data))
end

local function loadFromFile()
    if isfile(fileName) then
        local data = HttpService:JSONDecode(readfile(fileName))
        recordedPoints = {}
        for _, comp in ipairs(data) do table.insert(recordedPoints, CFrame.new(unpack(comp))) end
        return true
    end
    return false
end

-- ================= UI SETUP =================
local gui = Instance.new("ScreenGui", player.PlayerGui)
gui.Name = "SoraV5_AntiDetect"
gui.ResetOnSpawn = false

local frame = Instance.new("Frame", gui)
frame.Size = UDim2.fromOffset(260, 420)
frame.Position = UDim2.fromScale(0.5, 0.5)
frame.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
frame.Active = true
frame.Draggable = true
Instance.new("UICorner", frame)

local title = Instance.new("TextLabel", frame)
title.Size = UDim2.new(1, 0, 0, 35)
title.Text = "SORA RECORDER V5"
title.TextColor3 = Color3.fromRGB(0, 255, 180)
title.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
title.Font = Enum.Font.GothamBold
Instance.new("UICorner", title)

local status = Instance.new("TextLabel", frame)
status.Size = UDim2.new(1, 0, 0, 25)
status.Position = UDim2.new(0, 0, 0.94, 0)
status.Text = "Status: Ready"
status.TextColor3 = Color3.new(0.8, 0.8, 0.8)
status.BackgroundTransparency = 1
status.Font = Enum.Font.Gotham
status.TextSize = 10

-- BUTTON UTILS
local function createBtn(text, pos, color)
    local b = Instance.new("TextButton", frame)
    b.Size = UDim2.new(0, 220, 0, 32); b.Position = pos; b.AnchorPoint = Vector2.new(0.5, 0); b.Text = text; b.BackgroundColor3 = color; b.TextColor3 = Color3.new(1, 1, 1); b.Font = Enum.Font.GothamBold; b.TextSize = 12; Instance.new("UICorner", b)
    return b
end

local recBtn    = createBtn("RECORD POINT (0)", UDim2.new(0.5, 0, 0.1, 0), Color3.fromRGB(45, 45, 90))
local flyBtn    = createBtn("FLY: OFF", UDim2.new(0.5, 0, 0.2, 0), Color3.fromRGB(60, 60, 60))
local speedBtn  = createBtn("FLY SPEED: 50", UDim2.new(0.5, 0, 0.3, 0), Color3.fromRGB(140, 100, 20))
local delayBtn  = createBtn("RUSH DELAY: 10s", UDim2.new(0.5, 0, 0.43, 0), Color3.fromRGB(100, 60, 120))
local rushBtn   = createBtn("START TWEEN RUSH", UDim2.new(0.5, 0, 0.53, 0), Color3.fromRGB(0, 120, 120))
local saveBtn   = createBtn("SAVE TO FILE", UDim2.new(0.5, 0, 0.7, 0), Color3.fromRGB(50, 50, 50))
local clearBtn  = createBtn("CLEAR DATA", UDim2.new(0.5, 0, 0.82, 0), Color3.fromRGB(120, 30, 30))

-- TOGGLES
speedBtn.MouseButton1Click:Connect(function()
    currentSpeedIdx = (currentSpeedIdx % #speeds) + 1
    flySpeed = speeds[currentSpeedIdx]
    speedBtn.Text = "FLY SPEED: " .. flySpeed
end)

delayBtn.MouseButton1Click:Connect(function()
    currentDelayIdx = (currentDelayIdx % #delays) + 1
    rushDelay = delays[currentDelayIdx]
    delayBtn.Text = "RUSH DELAY: " .. rushDelay .. "s"
end)

-- FLY LOGIC (Improved for Anti-Reset)
local bv, bg
flyBtn.MouseButton1Click:Connect(function()
    flying = not flying
    flyBtn.Text = flying and "FLY: ON" or "FLY: OFF"
    flyBtn.BackgroundColor3 = flying and Color3.fromRGB(0, 150, 255) or Color3.fromRGB(60, 60, 60)
    
    if flying then
        local char = player.Character or player.CharacterAdded:Wait()
        local hrp = char:WaitForChild("HumanoidRootPart")
        local hum = char:WaitForChild("Humanoid")
        hum.PlatformStand = true
        bv = Instance.new("BodyVelocity", hrp); bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
        bg = Instance.new("BodyGyro", hrp); bg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge); bg.D = 100
        
        task.spawn(function()
            while flying and char.Parent do
                local camCF = workspace.CurrentCamera.CFrame
                local dir = Vector3.new()
                if UserInputService:IsKeyDown(Enum.KeyCode.W) then dir = dir + camCF.LookVector end
                if UserInputService:IsKeyDown(Enum.KeyCode.S) then dir = dir - camCF.LookVector end
                if UserInputService:IsKeyDown(Enum.KeyCode.A) then dir = dir - camCF.RightVector end
                if UserInputService:IsKeyDown(Enum.KeyCode.D) then dir = dir + camCF.RightVector end
                if UserInputService:IsKeyDown(Enum.KeyCode.Space) then dir = dir + Vector3.new(0,1,0) end
                if UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then dir = dir - Vector3.new(0,1,0) end
                bv.Velocity = dir * flySpeed
                bg.CFrame = camCF
                task.wait()
            end
            if hum then hum.PlatformStand = false end
            if bv then bv:Destroy() end
            if bg then bg:Destroy() end
        end)
    end
end)

-- AUTO RUSH TWEEN (Solusi agar Checkpoint terdeteksi)
rushBtn.MouseButton1Click:Connect(function()
    if #recordedPoints == 0 then status.Text = "No data!"; return end
    rushing = not rushing
    rushBtn.Text = rushing and "STOP RUSH" or "START TWEEN RUSH"
    
    if rushing then
        flying = false -- Matikan fly agar tidak tabrakan logic
        task.spawn(function()
            for i, targetCF in ipairs(recordedPoints) do
                if not rushing then break end
                local char = player.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                
                if hrp then
                    status.Text = "Moving to Pt " .. i .. " (Wait " .. rushDelay .. "s)"
                    
                    -- METODE TWEEN: Karakter meluncur (bukan teleport instan)
                    -- Durasi 0.5 detik agar sempat menyentuh hitbox checkpoint
                    local tween = TweenService:Create(hrp, TweenInfo.new(0.5, Enum.EasingStyle.Linear), {CFrame = targetCF})
                    tween:Play()
                    tween.Completed:Wait()
                    
                    -- Tunggu di checkpoint agar server mencatat
                    task.wait(rushDelay)
                end
            end
            rushing = false
            rushBtn.Text = "START TWEEN RUSH"
            status.Text = "Rush Done!"
        end)
    end
end)

-- HANDLERS LAIN
recBtn.MouseButton1Click:Connect(function()
    local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    if hrp then
        table.insert(recordedPoints, hrp.CFrame)
        recBtn.Text = "RECORD POINT (" .. #recordedPoints .. ")"
        status.Text = "Saved Point " .. #recordedPoints
    end
end)

saveBtn.MouseButton1Click:Connect(function() saveToFile(); status.Text = "File Saved!" end)
clearBtn.MouseButton1Click:Connect(function()
    recordedPoints = {}
    recBtn.Text = "RECORD POINT (0)"
    if isfile(fileName) then delfile(fileName) end
    status.Text = "Data Wiped!"
end)

if loadFromFile() then recBtn.Text = "RECORD POINT (" .. #recordedPoints .. ")"; status.Text = "Loaded Map Data" end
