-- =====================================================================
-- SORA HUB V6.7 - ULTIMATE INTEGRATED SYSTEM (GUI + CDN + AFK WEBHOOK)
-- =====================================================================
...
local lastCaughtWebhook = 0
local afkTimerWebhook = 0
local lastWebhookTimestamp = 0
...
local function sendToDiscord(title, message, playerName, color)
    ...
    local data = {
        ...
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "SORA HUB V6.7 | ULTIMATE MONITOR" },
...
-- ================= MAIN LOOPS & WEBHOOK INTEGRATION =================
...
RunService.RenderStepped:Connect(function()
    ...
    if hrp then 
        ...
        -- MY STATS & WEBHOOK LOGIC
        if statsData then 
            local currentCaught = statsData.Caught.Value
            if myStatsFrame.Visible then
                caughtVal.Text = formatFullNumber(currentCaught) 
                sessionVal.Text = "Session: +" .. formatFullNumber(currentCaught - initialCaught)
            end

            -- WEBHOOK AFK CHECK (Every 60s)
            if tick() - lastWebhookTimestamp >= 60 then
                lastWebhookTimestamp = tick()
                if currentCaught == lastCaughtWebhook then
                    afkTimerWebhook = afkTimerWebhook + 1
                    local t = afkTimerWebhook
                    if t == 1 then sendToDiscord("AFK WARNING (1M)", "AKUN @" .. player.Name:upper() .. " STUCK 1 MENIT", player.Name, 16776960)
                    elseif t == 10 then sendToDiscord("AFK WARNING (10M)", "AKUN @" .. player.Name:upper() .. " STUCK 10 MENIT", player.Name, 16753920)
                    elseif t == 30 then sendToDiscord("AFK WARNING (30M)", "AKUN @" .. player.Name:upper() .. " STUCK 30 MENIT", player.Name, 16737792)
                    elseif t == 60 then sendToDiscord("AFK WARNING (1H)", "AKUN @" .. player.Name:upper() .. " STUCK 1 JAM", player.Name, 16711680) end
                else
                    afkTimerWebhook = 0
                end
                lastCaughtWebhook = currentCaught
            end
        end
    end
...
-- PLAYER JOIN/LEAVE WEBHOOK
Players.PlayerAdded:Connect(function(plr) if targetData[plr.Name] then sendToDiscord("PLAYER JOINED", "AKUN @" .. plr.Name:upper() .. " TELAH MASUK SERVER", plr.Name, 65460) end end)
Players.PlayerRemoving:Connect(function(plr) if targetData[plr.Name] and plr ~= player then sendToDiscord("PLAYER LEFT", "AKUN @" .. plr.Name:upper() .. " KELUAR SERVER", plr.Name, 16724814) end end)

print("SORA HUB V6.7: LOADED SUCCESSFULLY.")
