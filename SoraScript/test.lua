...
-- TABLE UNTUK MONITORING ALL SERVER (WEBHOOK)
...
-- ================= MONITORING LOGIC (ALL SERVER STATS) =================
sendToDiscord("HOST ACTIVATE", "SYSTEM V6.8 CONNECTED. MONITORING ALL SERVER STARTED.", player.Name, 65460)
...
RunService.RenderStepped:Connect(function()
    ...
    if hrp then 
        ...
        -- ALL SERVER STATS LOGIC (Webhook)
                ...
                if targetData[p.Name] then
                    ...
                    if lStats then
                        ...
                        if lastCaughtAllServer[p.Name] ~= nil and lastCaughtAllServer[p.Name] == cVal then
                            ...
                            elseif t == 30 then sendToDiscord("AFK WARNING", "@" .. p.Name:upper() .. " STUCK 30 MIN", p.Name, 16737792)
                            elseif t == 60 then sendToDiscord("AFK WARNING", "@" .. p.Name:upper() .. " STUCK 1 HOUR", p.Name, 16711680) end
                        else
                            afkTimersAllServer[p.Name] = 0
                        end
