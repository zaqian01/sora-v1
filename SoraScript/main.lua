-- =================================================================
-- SORA HUB V5 FINAL + MONITORING SYSTEM (FINAL VERSION)
-- =================================================================
...
-- 2. DISCORD MONITORING CONFIGURATION (FINAL VERSION)
...
local function sendToDiscord(title, message, playerName, color)
    ...
    if playerName then
        avatarUrl = "https://www.roblox.com/headshot-thumbnail/image?userId=" .. (Players:GetUserIdFromNameAsync(playerName) or 0) .. "&width=420&height=420&format=png"
    end
    
    local data = {
        -- Hanya ngetag jika ID tersedia dan tidak kosong
        ["content"] = (discordId and discordId ~= "") and "<@" .. discordId .. ">" or "", 
        ["embeds"] = {{
            ["title"] = title,
            ["description"] = message,
            ["color"] = color or 65460,
            ["thumbnail"] = playerName and { ["url"] = avatarUrl } or nil,
...
-- 4. HOST SIGNALS (ASLI DARI SCRIPT MONITORING FINAL)
...
game:BindToClose(function()
    sendToDiscord("HOST DISCONNECT", "SISTEM PENGELOLAAN WEBHOOK TERPUTUS. HOST @" .. Players.LocalPlayer.Name:upper() .. " TELAH KELUAR.", Players.LocalPlayer.Name, 16724814)
end)

-- 5. ANTI AFK & PLAYER EVENTS
...
-- MONITOR AFK BERTINGKAT (FINAL LOGIC)
task.spawn(function()
...
RunService.Stepped:Connect(function()
    ...
    if autoXmas and HRP() then
        ...
        if isE and t.min == 0 and t.sec >= 30 and not teleported then
            lastPos = HRP().CFrame; HRP().CFrame = _G.SelectedEventSpot or CFrame.new(606, -580, 8923); teleported = true
